...
export default function EnhancedGPSTrackMap({
	...
	useEffect(() => {
		...
		const map = new OlMap({
			...
			layers: [baseLayer, racingLineLayer, selectedMarkerLayer],
			view: new View({
	...
	useEffect(() => {
		...
		try {
			...
			console.log(
				"Racing line updated successfully with",
				selectedMetric,
				"coloring",
			);
		} catch (error) {
	...
	useEffect(() => {
		...
		if (selectedPoint && selectedPoint.Lat && selectedPoint.Lon) {
			...
			const displayText = `${selectedPoint.LapDistPct.toFixed(1)}% - ${selectedMetric}: ${metricValue?.toFixed(1) ?? 1.0}`;
	...
	return (
		<div className="h-[500px] bg-gray-900 rounded-lg relative">
			...
			<div className="absolute bottom-2 left-2 z-10 bg-gray-800 bg-opacity-95 border border-gray-600 p-2 rounded text-white text-xs">
				...
				<div>
					Selected: {selectedPointIndex >= 0 ? selectedPointIndex : "None"}
				</div>
			</div>
