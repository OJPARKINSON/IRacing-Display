...
export default function TelemetryPage({ params }: Params) {
  ...
  const { dataWithGPSCoordinates, trackBounds, processError } = useGPSTelemetryData(
    telemetry,
    telemetry?.[0]?.TrackName
  );
    ...
    <div className="p-4 bg-gray-900 text-white min-h-screen">
      ...
      <div className="mb-4 bg-gray-800 p-4 rounded-lg">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <h3 className="text-lg font-semibold text-blue-400">Track Information</h3>
            <p className="text-gray-300">Name: {trackName}</p>
            <p className="text-gray-300">Session: {sessionNum}</p>
            {trackBounds && (
              <p className="text-gray-300 text-sm">
                GPS Bounds: {trackBounds.minLat.toFixed(4)}, {trackBounds.minLon.toFixed(4)} to {trackBounds.maxLat.toFixed(4)}, {trackBounds.maxLon.toFixed(4)}
              </p>
            )}
          </div>
          <div>
            <h3 className="text-lg font-semibold text-green-400">Data Quality</h3>
            <p className="text-gray-300">GPS Points: {dataWithGPSCoordinates.length}</p>
            <p className="text-gray-300">
              GPS Coverage: {telemetry ? ((dataWithGPSCoordinates.length / telemetry.length) * 100).toFixed(1) : 0}%
            </p>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-purple-400">Current Selection</h3>
            {selectedIndex >= 0 && dataWithGPSCoordinates[selectedIndex] && (
              <>
                <p className="text-gray-300">
                  Speed: {dataWithGPSCoordinates[selectedIndex].Speed?.toFixed(1) || 'N/A'} km/h
                </p>
                <p className="text-gray-300">
                  Position: {dataWithGPSCoordinates[selectedIndex].Lat?.toFixed(6)}, {dataWithGPSCoordinates[selectedIndex].Lon?.toFixed(6)}
                </p>
                <p className="text-gray-300">
                  Section: {(dataWithGPSCoordinates[selectedIndex] as any).sectionType || 'Unknown'}
                </p>
              </>
            )}
          </div>
...
function GPSAnalysisPanel({ data }: { data: any[] }) {
  const totalDistance = data.reduce((sum, point) => sum + (point.distanceFromPrev || 0), 0);
  const avgSpeed = data.reduce((sum, point) => sum + (point.Speed || 0), 0) / data.length;
    ...
    <div className="mt-4 bg-gray-800 p-4 rounded-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-gray-700 p-3 rounded">
          ...
          <div className="text-xl font-bold">{(totalDistance / 1000).toFixed(2)} km</div>
        </div>
        <div className="bg-gray-700 p-3 rounded">
        ...
        <div className="bg-gray-700 p-3 rounded">
          ...
          <div className="text-xl font-bold">{minSpeed.toFixed(0)} - {maxSpeed.toFixed(0)} km/h</div>
        </div>
        <div className="bg-gray-700 p-3 rounded">
          <div className="text-sm text-gray-400">Corner Points</div>
          <div className="text-xl font-bold">{corners.length}</div>
          <div className="text-xs text-gray-500">{((corners.length / data.length) * 100).toFixed(1)}% of lap</div>
        </div>
      </div>
    </div>
  );
